...
gulp.task('ad', sequence('compile-ad'));
...
gulp.task('minifyCSS', function () {
	...
	gulp.src('webapp/css/styles.ad.css')
		.pipe(minifyCSS())
		.pipe(gulp.dest('webapp/build/css/'));
});
...
gulp.task('compile-ad', function () {
	...
	return gulp.src('webapp/bower_components/almond/almond.js')
		.pipe(gulp.dest('webapp/build'))
		.pipe(rjs({
			out: 'require.config.ad.js',
			baseUrl: 'webapp/js/',
			name: 'require.config.ad',
			mainConfigFile: 'webapp/js/require.config.ad.js'
		}))
		.pipe(gulp.dest('webapp/build'))
		.on('end', function () {
			gulp.src('webapp/build/*.js')
				...
				.pipe(gulp.dest('webapp/build/js/'))
				.on('end', function () {
					del(['webapp/build/almond.js', 'webapp/build/require.js', 'webapp/build/require.config.ad.js']);
				});
		});
});
