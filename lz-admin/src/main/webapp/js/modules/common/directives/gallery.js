define(function () {
	return ['$document', '$timeout', '$q', '$templateCache', gallery];
	function gallery($document, $timeout, $q, $templateCache) {
		var defaults = {
		...
		$templateCache.put(template_url,
			...
			'    <image-viewer image-id="{{ i }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" ></image-viewer>' +
			'  </div>' +
			'</div>' +
			...
			'  <image-viewer image-id="{{ img }}" ng-click="nextImage()" ng-show="!loading" class="effect"></image-viewer>' +
			...
			'      <div ng-repeat="i in images">' +
			'        <image-viewer image-id="{{ i }}" ng-class="{\'active\': index === $index}" ng-click="changeImage($index)"></image-viewer>' +
			'      </div>' +
			'    </div>' +
			'  </div>' +
			'</div>'
