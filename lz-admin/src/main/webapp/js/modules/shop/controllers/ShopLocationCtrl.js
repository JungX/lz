define([], function () {
	return ['$scope', '$state', 'AlertService', 'ShopService', 'LEVELS', '$timeout', ShopLocationCtrl];

	function ShopLocationCtrl($scope, $state, AlertService, ShopService, LEVELS, $timeout) {
		var shopId = $state.params.shopId;
		$scope.levels = LEVELS;

		$scope.exit = function () {
			$state.go('main.shop.list')
		};
		...
		$scope.setLocation = function () {
			ShopService.setLocation($scope.data.id, $scope.data.coordinates).then(function () {
